 ...
 */
router.put('/webhook', verifyToken, validator(setWebhook), async (req: Request, res: Response): Promise<void> => {
  try {
    const tenantId = (req as any).user?.userId as string;
    const { webhook_url } = req.body || {};
    ConfigStore.upsertUserConfig(tenantId, { webhook_url: webhook_url ?? null });
    (res as any).sendResponse(200, { success: true, message: 'Webhook updated', webhook_url: ConfigStore.getWebhookUrl(tenantId) });
  } catch (error) {
    (res as any).sendError(500, error);
  }
});
